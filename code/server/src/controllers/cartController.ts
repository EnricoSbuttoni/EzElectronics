...
class CartController {
    ...
    async getCart(user: User): Promise<Cart> { 
        const cart1 =  this.dao.getCart(user.username); // Aspetta che la promise si risolva
        return cart1;
    }
    ...
    async getCustomerCarts(user: User): Promise<Cart[]> {
        const c= await this.dao.retrieveCarts(user.username);
        return c;
     } 
    ...
    async removeProductFromCart(user: User, product: string) /**Promise<Boolean> */ {
        return this.dao.removeFromCart(user.username,product);
     }
    ...
    async clearCart(user: User)/*:Promise<Boolean> */ { 
        return this.dao.removeAllfromCart(user.username);
    }
